...
public class JwtUtil {
     ...
     * @author wangh
     * @date 2020/5/30 17:26
     */
    public static String encode(String userId, String nickName, String secret, long timeOut) {
        Algorithm algorithm = Algorithm.HMAC256(secret);
        String token = JWT.create()
                //设置过期时间为一个小时
                .withExpiresAt(new Date(System.currentTimeMillis() + timeOut))
                //设置负载
                .withClaim(AppConstants.NICK_NAME, nickName)
                .withClaim(AppConstants.USER_ID, userId)
                .sign(algorithm);
        return token;
    }
